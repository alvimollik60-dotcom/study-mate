...
// --- ৩. Firebase লাইভ কাস্টম নোটিশ ইঞ্জিন ---
async function loadNoticeSettingsDisplay() {
    // ডিফল্ট ভ্যালু সেটআপ
    let text = 'Study mate অ্যাপের নিচের নোটিশ বক্সে আপনাকে স্বাগতম।';
    let color = '#ffffff';
    let size = '16';

    try {
        // ফায়ারবেস ডাটাবেজ থেকে রিয়েল-টাইমে নোটিশ ডাটা নিয়ে আসা
        ...
        console.error("নোটিশ ডাটাবেজ থেকে লোড করা যায়নি, লোকাল ডাটা ব্যবহার হচ্ছে:", e);
...
async function saveNoticeSettings() {
    ...
    try {
        // সরাসরি Firebase Firestore-এ নোটিশটি আপলোড ও সেভ করা
        const noticeDocRef = window.fbFirestore.doc(window.firebaseDb, "settings", "global_notice");
        await window.fbFirestore.setDoc(noticeDocRef, {
            text: textVal,
            color: colorVal,
            size: sizeVal,
            updatedAt: new Date()
        ...
        // ব্যাকআপ হিসেবে লোকাল স্টোরেজেও রাখা
        localStorage.setItem('notice_text', textVal);
        localStorage.setItem('notice_color', colorVal);
        localStorage.setItem('notice_size', sizeVal);

        await loadNoticeSettingsDisplay();
        alert('হোম পেজের লাইভ ডাটাবেজ নোটিশটি সফলভাবে আপডেট করা হয়েছে!');
    } catch (error) {
        console.error(error);
        alert('ফায়ারবেস ডাটাবেজে নোটিশ সেভ করা যায়নি!');
    }
}
...
async function login() {
    ...
    try {
        ...
        // ইউজারনেম 'admin' হলে তাকে ফুল অ্যাডমিন রোল দেওয়া হবে
        if (userVal === 'admin') {
...
function buildCalendar() {
    ...
    for (let day = 1; day <= totalDays; day++) {
        ...
        if (userDailyLogs[dateStr] && Object.values(userDailyLogs[dateStr]).some(v => v > 0)) { classes += ' has-data'; }
...
function loadReportForDate(dateStr) {
    ...
    activities.forEach((act) => {
        const sec = dayData[act] || 0; totalSeconds += sec;
    ...
    if(!chartEl) return;
    if (totalSeconds === 0) {
        chartEl.style.background = `conic-gradient(#334155 0% 100%)`; centerText.innerText = "No Data Available";
    } else {
        let currentPercent = 0; let gradientStops = [];
        chartValues.forEach((val, i) => {
            if (val > 0) {
                const startDeg = (currentPercent / totalSeconds) * 360; currentPercent += val;
                const endDeg = (currentPercent / totalSeconds) * 360; gradientStops.push(`${chartColors[i]} ${startDeg}deg ${endDeg}deg`);
            }
        });
        chartEl.style.background = `conic-gradient(${gradientStops.join(', ')})`; centerText.innerText = "Activity Breakdown";
    }
}